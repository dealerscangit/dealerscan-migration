// ============================================================
// DealerScan / Scan Docs — Apps Script (NEW for Workspace migration)
// Built: 2026-04-30 by Aria
// Source: Brandon's Code.gs from chat 2026-04-29, with corrections
//
// CHANGES FROM OLD:
...
//   5. Folder ID constants left as PLACEHOLDERS — Brandon fills before deploy
//
...
// salesperson Gmail accounts as in v3.9. After Phase 4A is stable, Phase 4B
// will refactor this file to mediate all Drive access through Apps Script.
// ============================================================
...
// Vision API key now read from Script Properties (set in Project Settings)
...
function doGet(e) {
  ...
  if (action === "getConfig")     return getDashConfigHttp(e); // Q-A fix
...
// ────────── CONFIG (Q-A FIX: HTTP wraps the plain object version) ──────────
function getDashConfigHttp(e) {
  try { return json(getDashConfig()); }
  catch(err) { return jsonError(err); }
}

// Plain object — used by google.script.run AND by the HTTP wrapper above
function getDashConfig() {
  ...
  // Auto-discover salesperson names from folder names in Drive
...
function createCustomerFolder(e) {
  ...
  if (e.parameter.isNew === "true") saveToHistory(customerName, salesName);
...
// Q-B FIX: removed the no-op col-8 setValue that raced with incrementPhotoCount
function logScanComplete(folderId) {
  ...
  for (var i = data.length - 1; i >= 0; i--) {
    if (data[i][2] === folderId) {
      ...
      // Q-B: was reading col 8 and writing it back — removed (incrementPhotoCount handles col 8)
...
// ────────── ARCHIVE / CLEANUP ──────────
// Q-C FIX: was named archiveDailyFolders. Renamed to make foot-gun nature explicit.
// Archives ALL folders unconditionally. Don't call from triggers. Don't call casually.
